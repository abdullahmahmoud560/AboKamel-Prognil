...
public interface IProductService : IApplicationService, IScopedService
{
    ...
    Task<ResultAbstract<List<ProductFavoriteDetailedResponseDto>>> SearchProductsAsync(ProductSearchRequestDto searchTerm, string userId);
    Task<ResultAbstract<List<ProductFavoriteResponseDto>>> GetAllProductsWithFavoriteAsync(string userId);
}